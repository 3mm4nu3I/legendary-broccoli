"""
OSINT Command Terminal — PROJECT NAPALM V3.1 (Singularity Final)
==========================================================================
Clearance: EID Verified (B. Noffels, 6/24/2026)
Architecture Upgrades:
- Hoisted `all_possible_tickers` definition: Eliminates price ticker scope crash.
- INTERACTIVE MAP PANNING [M]: W/A/S/D cursor movement & node inspection.
- EMBEDDED SQLITE DATABASE: Persistent local logging of all telemetry & trades.
- AUDIO/VISUAL THRESHOLD ALERTS: Native Windows audio triggers on $C_t$ spikes.
- RED/BLUE TEAM ADVERSARIAL AI: Dual-agent critique loop for investment advisories.
"""
...
DB_PATH = SCRIPT_DIR / "napalm_intel.db"
...
# ---------------------------------------------------------
# GLOBAL ENGINE STATE & THREAD LOCKS (HOISTED)
# ---------------------------------------------------------
current_view = "1"
...
# ---------------------------------------------------------
# MARKETS & TICKERS DEFINITION (HOISTED)
# ---------------------------------------------------------
MARKETS = {
...
def render_interactive_ascii_map(level, war_snap, infra_snap):
    ...
    sitrep = f"\n{PURPLE}  --- INTERACTIVE MAP CONTROLS: W/A/S/D (Move) | ENTER (Inspect) | Z/X (Zoom) ---{RESET}\n"
    sitrep += f"  {CYAN}CURSOR POS: [{map_cursor_row}, {map_cursor_col}] | NODE INSPECT: {selected_node_info}{RESET}\n"
...
def run_napalm_engine(composite, C_t, K, E_t, T_t, war_snap, infra_snap, ins_detail_snap, p_snap, osint_snap):
    ...
    prompt = f"""You are PROJECT NAPALM Singularity. Generate a dual-agent adversarial review (Blue Team Bullish Thesis vs Red Team Bearish Critique) based on this telemetry:
...
def main():
    ...
    print(f"{CYAN} PROJECT NAPALM V3.1 — SINGULARITY FIXED | ALL PILLARS ACTIVE {RESET}")
    print(f"{PURPLE}========================================================================={RESET}")
    ...
    try:
        while True:
            ...
            if view_key in MARKETS:
                ...
                print(f"\n{BLUE}========================================================================================={RESET}")
                print(f"{PURPLE}  WORLDWIDE OSINT HEADLINE RADAR (Persistent SQLite Database){RESET}")
                print(f"{BLUE}========================================================================================={RESET}")
                ...
                print(f" Active Alerts        : {len(infra_snap)} verified events in SQLite persistent database.\n")
                if not infra_snap:
                ...
                print(f"{PURPLE}  PROJECT NAPALM: RED/BLUE ADVERSARIAL NEURAL ENGINE{RESET}")
                print(f"{BLUE}========================================================================================={RESET}")
                print(f" {YELLOW}{PREDICTION_DISCLAIMER}{RESET}")
                ...
                if not napalm_advisories:
                    print(f" {GRAY}Awaiting dual-agent JSON response from NAPALM engine...{RESET}")
                else:
            ...
            print(f"\n{BLUE}========================================================================================={RESET}")
            print(f"{PURPLE}  NAPALM V3.1 SINGULARITY STATUS (Composite Index: {global_composite:.2f} | C_t: {global_C_t:.2f}){RESET}")
            print(f"{BLUE}========================================================================================={RESET}")
        ...
        print(f"\n{CYAN}[+] NAPALM V3.1 Singularity terminal disengaged safely by operator.{RESET}")
