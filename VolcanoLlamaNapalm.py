"""
OSINT Command Terminal — PROJECT NAPALM V2.2 (Autonomous Self-Healing)
==========================================================================
Clearance: EID Verified (B. Noffels, 6/24/2026)
Architecture Upgrades:
- AUTONOMOUS OLLAMA BOOTSTRAPPER: Automatically scans drives (F:, C:, etc.) for ollama.exe and launches it.
...
def auto_bootstrap_ollama():
    """Scans standard paths and system drives for ollama.exe and launches it if offline."""
    ...
    # Search paths including F: drive
    possible_paths = [
        r"C:\Program Files\Ollama\ollama.exe",
        os.path.expandvars(r"%LOCALAPPDATA%\Programs\Ollama\ollama.exe"),
        r"F:\Ollama\ollama.exe",
        r"F:\Program Files\Ollama\ollama.exe",
    ...
    if not ollama_bin:
        for drive in ["C:", "D:", "E:", "F:", "G:"]:
            target = os.path.join(drive, "\\", "Program Files", "Ollama", "ollama.exe")
...
def main():
    ...
    print(f"{CYAN} PROJECT NAPALM V2.2 | AUTONOMOUS OLLAMA BOOTSTRAPPER | DYNAMIC MAPS {RESET}")
    print(f"{PURPLE}========================================================================={RESET}")
    
    # Auto-bootstrap Ollama before entering main UI loop
    auto_bootstrap_ollama()
    ...
    try:
        while True:
            ...
            print(f"\n{BLUE}========================================================================================={RESET}")
            print(f"{PURPLE}  NAPALM V2.2 STATUS (Composite Index: {global_composite:.2f} | C_t: {global_C_t:.2f}){RESET}")
            print(f"{BLUE}========================================================================================={RESET}")
        ...
        print(f"\n{CYAN}[+] NAPALM V2.2 terminal disengaged safely by operator.{RESET}")
