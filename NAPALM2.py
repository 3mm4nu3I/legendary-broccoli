"""
OSINT Command Terminal — PROJECT NAPALM V2.1 (The Final Overhaul)
==========================================================================
Clearance: EID Verified (B. Noffels, 6/24/2026)
Architecture Upgrades:
- DYNAMIC ALL-POINTS TACTICAL MAP [M]: Real-time plotting of ALL active telemetry nodes.
- INTERACTIVE GRAPH MATRIX [G]: Multi-option overlays (Momentum, Volume Z-Score, Friction).
- INSTANT WEATHER & FAA REFRESH: Sub-15s polling for meteorological & airspace alerts.
- BULLETPROOF SEC EDGAR: Multi-stage XML extraction with local state persistence.
"""
...
FAST_POLL_INTERVAL_SECONDS = 15.0 # Instant Weather/FAA refresh
...
zoom_level = 0
graph_overlay_mode = 1 # 1: Momentum, 2: Volume Z-Score, 3: Friction Impact
...
def render_ascii_map(level, war_snap, infra_snap, weather_snap):
    ...
    # Scan war & infra buffers for location keywords
    coords_map = {
    ...
    # Plot nodes dynamically onto ASCII grid
  ...
  "advisories": [
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=OLLAMA_TIMEOUT_SECONDS) as response:
            ...
            validated = [adv for adv in raw_advisories if isinstance(adv, dict) and required_keys.issubset(adv.keys())]
            napalm_advisories = validated
    except Exception as e:
...
def main():
    ...
    print(f"{CYAN} PROJECT NAPALM V2.1 | DYNAMIC MAPS | INSTANT WEATHER | SEC EDGAR CORE {RESET}")
    print(f"{PURPLE}========================================================================={RESET}")
    print(f"\n[+] FTC Clearance Acknowledged: Bram Noffels (6/24/2026).")
    print(f"[+] Fast-Poll Daemon Active: Weather & FAA updating every 15s.")
    print(f"[+] SEC EDGAR Bulletproof XML Parser Deployed.")
    print(f"[+] PROJECT NAPALM AI Engine Initialized (Ollama: {OLLAMA_MODEL} @ {OLLAMA_ENDPOINT})")
    ...
    try:
        while True:
            ...
            if view_key in MARKETS:
                ...
                print(f"\n{BLUE}========================================================================================={RESET}")
                print(f"{PURPLE}  SEC EDGAR INSIDER DISCLOSURES (Form 4) — see [S] for real $ detail{RESET}")
                print(f"{BLUE}========================================================================================={RESET}")
            ...
            print(f"\n{BLUE}========================================================================================={RESET}")
            print(f"{PURPLE}  NAPALM V2.1 STATUS (Composite Index: {global_composite:.2f} | C_t: {global_C_t:.2f}){RESET}")
            print(f"{BLUE}========================================================================================={RESET}")
        ...
        print(f"\n{CYAN}[+] NAPALM V2.1 terminal disengaged safely by operator.{RESET}")
